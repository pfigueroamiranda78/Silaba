...
struct FileServiceProvider: FileService {
    ...
    func uploadVideo(data: FileServiceImageUploadData, track: ((Progress?) -> Void)?, callback: ((FileServiceResult) -> Void)?) {
        ...
        guard let videoUrl = data.localUrl else {
        ...
        let imagePath = "\(userId)/posts/\(key)."+videoUrl.pathExtension!
        let metadata = StorageMetadata()
        metadata.contentType = "video"
        
        let task = storageRef.child(imagePath).putData(imageData, metadata: metadata) { (metadata, error) in

            guard error == nil else {
                result.error = .failedToUpload(message: "Failed upload Video."+(error?.localizedDescription)!)
            ...
            guard let imageUrl = metadata?.downloadURL()?.absoluteString else {
                result.error = .failedToUpload(message: "Video URL does not exist.")
                callback?(result)
                return
            }
            ...
            let data: [String: AnyObject] = [
                ...
                "isVideo": data.isVideo as AnyObject,
