//
//  VideoPostUploadInteractor.swift
//  Silaba
//
...
class VideoPostUploadInteractor: VideoPostUploadInteractorInterface {
    
    weak var output: VideoPostUploadInteractorOutput?
...
extension VideoPostUploadInteractor: VideoPostUploadInteractorInput {
    
    func upload(with data: FileServiceImageUploadData, content: String, machineLearningList: MachineLearningList, talent: Talent) {
        
        fileService.uploadVideo(data: data, track: { (progress) in
            guard progress != nil else {
                return
            }
            
            self.output?.didUpdateVideo(with: progress!)
            
        }) { (result) in
            guard let fileId = result.fileId,
                let fileUrl = result.fileUrl,
                result.error == nil else {
                    self.output?.didFailVideo(with: result.error!.message)
                    return
            }
            ...
            self.postService.writePost(photoId: fileId, content: content, imageReconized: machineLearningList, talent: talent, callback: { (result) in
                ...
                guard let posts = result.posts,
                    ...
                    let (post, user) = posts[0] else {
                        self.output?.didFailVideo(with: "No se encontraron publicaciones")
                        return
                }
                ...
                self.output?.didSucceedVideo(with: uploadedPost)
